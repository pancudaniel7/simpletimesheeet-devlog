...
Go is not free of cost, and it would be dishonest to pretend otherwise. It is verbose, you write `if err != nil` a great many times, and there are days that hand-holding feels like noise rather than safety. Its generics are still young compared to languages that have had them for decades. And I chose a framework that sits on fasthttp instead of the standard library, which means a subset of the ecosystem's middleware works out of the box, a cost I pay knowingly for an API I prefer writing against.
